...
n_a = 64
x_initializer = torch.zeros((1, 1, 90))
...
def generate_music(inference_model, corpus = corpus, abstract_grammars = abstract_grammars, tones = tones, tones_indices = tones_indices, indices_tones = indices_tones, T_y = 10, max_tries = 1000, diversity = 0.5):
    ...
    for i in range(1, num_chords):
        ...
        pred = [indices_tones[p] for p in indices[0]]
...
def predict_and_sample(inference_model, x_initializer = x_initializer, a_initializer = a_initializer, 
    ...
    for i in range(30):
        out,(h,c) = inference_model(x_initializer,h,c)
        if(i==0):
            res = out
            continue
        res = torch.cat([res,out],dim=1)
    indices = torch.argmax(res, axis = -1)
    import torch.nn.functional as F
    results = F.one_hot(indices, num_classes=90)
